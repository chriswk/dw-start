...
/**
 * Taken from http://crackstation.net/hashing-security.htm#javasourcecode
...
public final class PasswordUtil {
    ...
    private static byte[] pbkdf2(final char[] password, final byte[] salt, final int iterations, final int bytes) {
        try {
            PBEKeySpec spec = new PBEKeySpec(password, salt, iterations, bytes * I_8);
    ...
    private static byte[] fromHex(final String hex) {
        ...
        for (int i = 0; i < binary.length; i++) {
            binary[i] = (byte) Integer.parseInt(hex.substring(2 * i, 2 * i + 2), I_16);
        }
        return binary;
    }
